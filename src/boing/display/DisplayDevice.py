# -*- coding: utf-8 -*-
#
# boing/display/DisplayDevice.py -
#
...
if not USING_PYLIBPOINTING:
    ...
    class DummyDisplayDevice(DisplayDevice):
        def __init__(self, url=None):
            ...
            w = float(url.query.data.get('w', 0))
            h = float(url.query.data.get('h', 0))
            self._size = Size(w, h)
            bx = float(url.query.data.get('bx', 0))
            by = float(url.query.data.get('by', 0))
            bw = float(url.query.data.get('bw', 0))
            bh = float(url.query.data.get('bh', 0))
            self._bounds = Bounds(bx, by, bw, bh)
            ppi = float(url.query.data.get('ppi', 75))
            self._resolution = (ppi, ppi)
            hz = float(url.query.data.get('hz', 0))
...
if __name__=="__main__":
    display = DisplayDevice.create("any:")
    print("Default display device:")
    display.debug()
